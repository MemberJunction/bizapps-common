...
-- fn_organization_parent_id_get_root_id: CodeGen's canonical recursive root lookup (scalar form).
-- Coexists with the converter-emitted fnOrganizationParentID_GetRootID above; the canonical
-- vwOrganizations below references this one, matching what CodeGen maintains.
CREATE OR REPLACE FUNCTION __mj_bizappscommon.fn_organization_parent_id_get_root_id(p_record_id uuid, p_parent_id uuid)
 RETURNS uuid
 LANGUAGE sql
 STABLE
AS $function$
...
AS SELECT o."ID",
   ...
   FROM __mj_bizappscommon."Organization" o
     ...
     LEFT JOIN LATERAL ( SELECT __mj_bizappscommon.fn_organization_parent_id_get_root_id(o."ID", o."ParentID") AS root_id) root_parentid ON true$vsql$;
  v_target_oid OID;
  v_dep RECORD;
  v_captured JSONB[] := ARRAY[]::JSONB[];
  v_n INTEGER;
...
-- References LinkedUserID (added in V202605201354); safe here because plpgsql bodies are not
...
-- References LinkedUserID (added in V202605201354); safe here — see spCreatePerson note.
