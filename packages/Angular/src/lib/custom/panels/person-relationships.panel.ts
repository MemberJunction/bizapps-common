...
export class PersonRelationshipsPanel extends BizAppsFormPanel<mjBizAppsCommonPersonEntity> implements OnInit {
    private static readonly PREF_KEY = 'mj.bizapps.common.person.relationshipViewMode';
    public ViewMode: 'list' | 'graph' = 'list';

    public ngOnInit(): void {
        const saved = UserInfoEngine.Instance.GetSetting(PersonRelationshipsPanel.PREF_KEY);
    ...
    public SetViewMode(mode: 'list' | 'graph'): void {
        this.ViewMode = mode;
        UserInfoEngine.Instance.SetSettingDebounced(PersonRelationshipsPanel.PREF_KEY, mode);
    }
}
