...
---------------------------------------------------------------------------
-- Note: vwPeople layered base view is defined after vwPeopleGenerated
-- (down in the view generation section) so g.* binds to SeniorityLevelID
-- and SeniorityLevel from the regenerated vwPeopleGenerated.
---------------------------------------------------------------------------
...
SET @SeniorityLevelEntityID = COALESCE(@SeniorityLevelEntityID, 'adcd72cb-b817-4fbc-97a4-80ee770376d1');
...
SET @JobFunctionEntityID = COALESCE(@JobFunctionEntityID, 'b453313c-d2fe-4bb9-9e6f-1d048495299d');

IF @PersonEntityID IS NULL OR @RelationshipEntityID IS NULL OR @SeniorityLevelEntityID IS NULL OR @JobFunctionEntityID IS NULL
BEGIN
    THROW 50002, 'Required EntityID could not be resolved in migration', 1;
END
...
-- 2. Person: SeniorityLevel (virtual display name)
IF @PersonEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
     WHERE [EntityID] = @PersonEntityID AND [Name] = 'SeniorityLevel'
)
BEGIN
    INSERT INTO [${mjSchema}].[EntityField] (
        [ID], [EntityID], [Sequence], [Name], [DisplayName],
        ...
        'a39f6723-5e28-4ad0-b8f4-4a259c78d101',
        @PersonEntityID,
        (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @PersonEntityID),
        'SeniorityLevel', 'Seniority Level',
        'Current primary seniority level name for this person.', 'nvarchar', 100, 0, 0,
        1, NULL, 0, 0,
        1, 0, NULL, NULL,
        0, 0, 0,
        0, 0, 0, 'Search',
...
-- 3. Person: PrimaryJobFunctionID (virtual FK)
IF @PersonEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
...
-- 4. Person: PrimaryJobFunction (virtual nvarchar)
IF @PersonEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
...
-- 5. Relationship: JobFunctionID (physical FK)
...
-- 6. Relationship: JobFunction (virtual display name)
IF @RelationshipEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
     WHERE [EntityID] = @RelationshipEntityID AND [Name] = 'JobFunction'
)
BEGIN
    INSERT INTO [${mjSchema}].[EntityField] (
        [ID], [EntityID], [Sequence], [Name], [DisplayName],
        ...
        'b51f8a42-7c39-4be1-c9d5-5b360d89e202',
        @RelationshipEntityID,
        (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @RelationshipEntityID),
        'JobFunction', 'Job Function',
        'Company-specific job function name for this relationship/employment link.', 'nvarchar', 100, 0, 0,
        1, NULL, 0, 0,
        1, 0, NULL, NULL,
        0, 0, 0,
        0, 0, 0, 'Search',
...
-- 7. Relationship: SeniorityLevelID (physical FK)
IF @RelationshipEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
...
-- 8. Relationship: SeniorityLevel (virtual display name)
IF @RelationshipEntityID IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM [${mjSchema}].[EntityField]
     WHERE [EntityID] = @RelationshipEntityID AND [Name] = 'SeniorityLevel'
)
BEGIN
    INSERT INTO [${mjSchema}].[EntityField] (
        [ID], [EntityID], [Sequence], [Name], [DisplayName],
        ...
        'c62a9b53-8d4a-4cf2-dae6-6c471e9af303',
        @RelationshipEntityID,
        (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @RelationshipEntityID),
        'SeniorityLevel', 'Seniority Level',
        'Company-specific seniority level name for this relationship/employment link.', 'nvarchar', 100, 0, 0,
        1, NULL, 0, 0,
        1, 0, NULL, NULL,
        0, 0, 0,
        0, 0, 0, 'Search',
...
BEGIN
    THROW 50001, 'Missing EntityField detected on Person or Relationship after migration', 1;
END

-- Assert no phantom EntityFields for Person or Relationship
IF EXISTS (
    SELECT 1
      FROM [${mjSchema}].[Entity] e
...
BEGIN
    THROW 50001, 'Phantom EntityField detected on Person or Relationship after migration', 1;
END
GO
