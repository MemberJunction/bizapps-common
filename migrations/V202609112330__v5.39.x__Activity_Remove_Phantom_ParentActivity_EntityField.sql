...
--
-- DO NOT GENERALISE THIS INTO "REMOVE ANY FIELD WITH NO VIEW COLUMN". That rule would be WRONG
-- roughly as often as it is right, and #117 is the proof: `ActivitySyncRunDetail.ActivitySyncExclusion`
-- was the same mismatch pointing the other way — the migration that shipped the view column deliberately
-- included the join, and the fix there is to ADD the missing field, not to drop the column. Applying
-- this file's direction there would have deleted a field somebody meant to exist and re-broken every
-- Run Detail save.
--
-- What makes removal right HERE and not there: `vwActivities` is regenerated by CodeGen WITHOUT a
-- ParentActivity column, verified by running a full `mj codegen` on a healed database and watching the
-- field stay gone and the counts stay 33/33. A migration that added the column would be undone on the
-- next CodeGen pass. Where the view genuinely carries the column, the field is what is missing.
--
-- So this migration names ONE field on ONE entity, matches `BaseTable = 'Activity'` exactly, and its
-- closing assertion checks `vwActivities` alone. It cannot reach another entity's metadata, by
